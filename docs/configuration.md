...
默认配置只监听本机，并使用确定性本地引擎。页面标注为“模板演示 · 未调用大模型”：没有模型
型号，也不消耗模型 Token；服务端仍负责项目、资料、母稿和版本的持久化。在线来源导入和文献
查询是独立的主动联网操作，不随演示模式关闭：
...
环境。任务只有在用户启动真实模型步骤时才向所选 Provider 发送相应内容。实际供应商和型号以
结果的 `execution.provider` / `execution.model` 为准，勿把路由画像名当成已调用模型。

### 页面连接与功能边界

- 单篇起草、标题实验室、润色和单篇审校支持“模板演示”或“模型 API”；API 可使用本页临时
  连接，也可在未填写本页连接时采用服务端默认模型。临时密钥只留在当前页面内存，不随项目保存。
- 项目工作流、直接母稿、渠道变体与项目模型增强审校只使用服务端模型；浏览器临时连接不替代
  `YANZHANG_LLM_*`。页面模型名和供应商说明将这两条连接路径区分显示。
- v2 HTTP/MCP 的工作流创建、母稿、变体、模型增强审校以 `live=false` 为默认。显式 `true` 才
  请求模型；服务端未配置时返回 `model_configuration_error`（HTTP 422），不会静默退回模板稿。
- 项目表达焦点及学术标题、提纲、摘要、引用核验、完整性检查和审稿回复等当前使用规则引擎。
  切换到“模型 API”不代表所有工具都调用模型。
- 新生成结果记录执行来源；手工修订和缺少历史记录的版本以 `execution=null` 表示，页面不根据
  当前设置猜测旧稿所用模型。

状态接口只公开连接是否就绪、供应商名、默认模型和演示引擎信息，不返回密钥或基础地址。

### 场景配置与材料选择

场景写法、语气、字段提示、示例与自检清单来自代码中的类型化目录，而非模型环境变量。
维护入口为 `yanzhang_core/scenario_profiles.py`；配方仍由 `yanzhang_core/packs.py` 管理。
修改后运行 `uv run python scripts/build_scenario_catalog.py`，部署前用同一命令追加 `--check`
执行只读一致性校验。浏览器生成目录和服务端代码应作为同一个版本部署，避免新旧目录混用。

选择写法卡只选择结构方法，不代表已经检索文章或文献。公文党报样文只供表达参考；职场、内容
传播与学术场景使用各自的资料入口。学术任务中的书目元数据、用户导入的正文和证据摘录与党政
样文分开传递；缺少研究证据时标记待补。切换项目或场景后重新核对本次关联资料，旧请求结果
不会自动写入新任务。此隔离针对页面工作态，已保存项目资料的访问边界仍由服务端项目 ID 校验。
职场、传播材料和团队范例需手工导入；学术文献、原文与证据支持导入，书目查询仍使用既有公开
连接器。本轮未新增商业媒体自动采集入口，选择场景也不触发自动来源检索。
