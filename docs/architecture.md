...
四个场景包共 19 个配方及 26 张写法方法卡：公文 6、职场 7、传播 6、学术 7。
方法卡描述表达方法，配方描述章节结构；两者的数量不等同。
`leadership-speech` 对应领导讲话，`research-report` 对应调研报告。
每个配方声明所需输入、章节用途、适用渠道、默认标题类型、输出格式和事实策略。任务简报必须与
所选场景包、配方和渠道匹配；无模型配置时，同一配方也能产生确定性本地草稿，便于离线验收。

### 场景目录的单一来源

`yanzhang_core/scenario_profiles.py` 定义四个类型化 `ScenarioProfile`：写法卡、语气、默认值、
资料入口、字段文案、占位提示、示例、自检清单、检查展示维度和模型提示约束。配方的权威定义仍
位于 `yanzhang_core/packs.py`；`scenario_catalog()` 从该注册表派生配方，不再复制一套定义。

`scripts/build_scenario_catalog.py` 将公开静态目录生成为
`gongwen_web/static/scenario_catalog.js`。浏览器 `workspace_context.js` 从生成目录的 `recipes`
派生配方、文种、渠道和内容结构，页面从 `profiles` 选择对应场景表现。修改场景或配方后运行：
...
生成过程只读取代码中的公开目录，不读取用户项目、模型密钥或来源全文，也不访问网络。
`--check` 在生成文件缺失或过期时返回非零状态，开发和发布检查应执行它。生成文件随发行包
一起交付，勿手工修改或在前端重新声明配方。兼容单篇生成、拟题、改写、审校及核心组合器共用
场景指引；旧文种通过统一解析映射到场景，避免仅更换界面标签却保留公文提示。
兼容 `methodology_catalog` 从全部核心配方派生 `recipe-<recipe_id>` 内容结构，同时保留原有
兼容结构方法。非党政配方的默认标题使用各自的 5 类公式，不继承党报标题或写法参考；因此
“26 张方法卡”不是该兼容接口的 `content_methodologies` 总数。

### 浏览器上下文与资料隔离

页面将项目、场景、配方及其资料选择视为一组写作上下文。场景切换同步更新写法、语气、示例、
资料入口与自检清单；党政媒体样文留在公文写法路径，职场/传播资料和学术文献证据走各自入口。
切换不删除已保存的项目资料，但旧场景候选、执行标记、审校和草稿不自动成为新任务结果。
职场业务资料、团队范例与传播素材由用户手工导入；学术文献和原文证据使用导入入口，公开书目
查询沿用既有连接器。本次场景重构未添加商业媒体自动采集适配器。

异步操作在发起时记录项目、场景和任务输入标识；应用响应前再核对上下文。检索、资料读取、
生成或保存期间发生切换时，迟到结果不得覆盖新任务。界面显示的所选资料与发给生成器的资料
集合须一致，风格样文和事实证据始终保持不同角色。
...
- `ExecutionInfo` 包含 `mode`、`engine`、`provider`、`model`、`label` 和 `uses_model`，记录实际
  执行引擎而非路由画像。`Revision.execution` 是可选字段，复用 `revisions.metadata_json` 的
  `execution` 成员存储，无需新增数据库列或提升 schema 版本。生成的首版保存当次执行信息；
  手工修订和缺少执行信息的旧版本为 `null`，不继承或猜测模型来源。资产读取按所选版本返回来源。
...
模板演示使用内置公式、确定性组合器和规则评分，没有调用任何大模型，不消耗模型 Token。
演示流程仍可通过本地服务持久化；来源导入和公开文献查询属于独立网络操作，模式标签不承诺
全功能断网。

`CreateWorkflowRequest.live` 默认 `false`。创建工作流时验证显式实时请求、解析路由并保存
`live` 与 `execution`，运行和恢复沿用该输入；缺少模型配置返回 `model_configuration_error`
（HTTP 422），不静默降级。工作流标题、提纲和自动审校步骤仍为本地规则；母稿 `draft` 才按
保存的模式选择组合器引擎，上述四类步骤记录自己的执行来源。直接母稿、渠道变体和独立项目模型增强
审校同样要求 `live=true`，并只使用服务端配置。

兼容页面的单篇起草、标题实验室、润色和单篇审校可选择本页临时 API 或服务端默认模型；临时
凭据不进入项目工作流。项目表达焦点，以及学术标题、提纲、摘要、引用检查、完整性检查和回复
生成等当前仍为规则功能。全局 API 设置表示可用连接，不表示所有工具均调用模型。

`execution` 用实际供应商和模型标识结果，`resolved_route` 仅解释路由画像；两者分开返回。
无型号声明的注入回调明确标注未知，历史结果也不根据当前环境补填。
...
- 当前学术生成关联本项目书目元数据及用户导入的原文或正文摘录；元数据用于识别来源，正文
  与有定位的证据用于支撑研究论断。材料缺失时保留研究框架、检索问题与待补位置，不编造作者、
  年份、DOI、样本、实验或统计结论。获取一条书目不表示系统已获取或阅读全文。
- `create_outline` 根据 `ResearchBrief.document_type` 区分综述、研究提纲、摘要、审稿回复四类
  任务结构；通用研究论文仍为六章。`AcademicOutline.task_type` 为 `literature-review`、
  `research-outline`、`abstract`、`rebuttal`、`research-paper` 五值枚举；
  `AbstractDraft.task_type` 固定为 `abstract`。非空 `journal.required_sections` 在领域调用中
  优先于默认章节，HTTP/MCP 提纲请求当前未暴露该期刊参数。页面摘要和逐条审稿回复分别使用
  既有摘要与回复接口，结果形态与任务保持一致。
- 核心组合器识别浏览器序列化的学术材料包，先提取标记的原文证据块，再处理一般上下文，避免
  前置书目清单或任务说明占满有限的摘录位置。本地组合的 480 字符摘录上限只作用于证据正文，
  文献/证据 ID 标记和材料包中已有的完整定位行独立保留。截断时在原文节选前单列提示：
  “下文已截断，不代表完整引文；请回查原始证据及上下文。”提示不混入原文。
  包说明和元数据不被编入正文论证。真实模型提示将书目信息单列为
  `bibliographic_metadata`，`knowledge` 保留原文证据和用户研究材料，两者承担不同角色。
- 学术组织方法和示例是内置结构指引，不是文献检索结果。场景适配不等于自动全文抓取、目标
  期刊精确排版或真实模型质量已验证；这些能力需要对应来源、模板或评测单独验证。
