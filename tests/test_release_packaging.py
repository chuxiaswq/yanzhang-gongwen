...
def test_distribution_metadata_is_standalone_and_pep639() -> None:
    ...
    assert "pip-audit>=2.10,<3" in dev_dependencies
    assert "pytest>=9.0.3,<10" in dev_dependencies
    assert "pytest-asyncio>=1,<2" in dev_dependencies
...
def test_connector_archive_is_deterministic_and_versioned(tmp_path: Path) -> None:
    ...
    archive = tmp_path / "yanzhang-workbuddy-connector-0.2.0-preview.2.zip"
    ...
    assert metadata["version"] == "0.2.0-preview.2"
    skill = (ROOT / "integrations/workbuddy-gongwen/skills/gongwen/SKILL.md").read_text(
        encoding="utf-8"
    )
    assert "version: 0.2.0-preview.2" in skill
...
def test_github_workflows_pin_actions_and_audit_every_archive() -> None:
    ...
    for workflow in (ci, release):
        ...
        assert "uv export --frozen --all-extras --no-emit-project" in workflow
        assert "uv run pip-audit --requirement" in workflow
